...
fn main() -> anyhow::Result<()> {
    ...
    */
    i2c.write(SENSOR_ADDR_GY273, &[0x0A, 0b10_10_00_11], I2C_Timeout).map_err(|e| anyhow::anyhow!("i2c write error"))?;
    // Register 0x0B: SOFT_RST=0 (no reset), SELF_TEST=0 (disabled), RNG<1:0>=10 (8 Gauss), 
    // SET/RESET MODE<1:0>=00 (set and reset on)
    i2c.write(SENSOR_ADDR_GY273, &[0x0B, 0b0000_10_00], I2C_Timeout).map_err(|e| anyhow::anyhow!("i2c write error"))?;
    
    // MPU Configuration
    // Reset / Wake up
    i2c.write(SENSOR_ADDR_MPU, &[0x1c, 0x00], I2C_Timeout)?;
    ...
    for _ in 0..100 {
        ...
        let acc_x = i16::from_be_bytes([buffer[0], buffer[1]]) as f32 / 16384.0;
        let acc_y = i16::from_be_bytes([buffer[2], buffer[3]]) as f32 / 16384.0;
        let acc_z = i16::from_be_bytes([buffer[4], buffer[5]]) as f32 / 16384.0;
    ...
    let acc_y_offset = acc_y_sum / 100.0;
    ...
    loop {
        ...
        let acc_x = i16::from_be_bytes([buf[0],buf[1]]) as f32 / 16384.0 - acc_x_offset;
        let acc_y = i16::from_be_bytes([buf[2],buf[3]]) as f32 / 16384.0 - acc_y_offset;
        let acc_z = i16::from_be_bytes([buf[4],buf[5]]) as f32 / 16384.0 - acc_z_offset;
        ...
        let temperature_c = (temperature / 340.0) + 36.53; // °C
        let acc_total = (acc_x * acc_x + acc_y * acc_y + acc_z * acc_z).sqrt();
        let roll = (acc_y / acc_total).asin() * 180.0 / PI;
        let pitch = -(acc_x / acc_total).asin() * 180.0 / PI;

        info!("Roll: {:.2} Pitch: {:.2} Temp: {:.2}°C",
              roll, pitch, temperature_c);
